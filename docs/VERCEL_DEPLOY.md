...
Реквизиты продавца (самозанятая, НПД) — в [`src/lib/legal/seller-info.ts`](../src/lib/legal/seller-info.ts). ОГРНИП не используется. Перед долгосрочной эксплуатацией рекомендуется юридическая проверка текстов оферты и политики конфиденциальности.
