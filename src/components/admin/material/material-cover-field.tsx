...
import { uploadMaterialCoverAction } from "@/app/actions/admin/media";
import { Button } from "@/components/ui/button";
import { resolveMaterialCoverUrl } from "@/lib/catalog/material-cover";
import { cn } from "@/lib/utils";

type MaterialCoverFieldProps = {
...
export function MaterialCoverField({
  ...
  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    startUpload(async () => {
      const result = await uploadMaterialCoverAction(formData);
  ...
  return (
    <div className="flex flex-col gap-2">
      <span className="text-sm font-medium text-foreground">Обложка материала</span>

      <div
        className={cn(
          "relative aspect-[2/1] w-full max-w-xl overflow-hidden rounded-xl bg-neutral-100",
      ...
      >
        {coverUrl ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 px-6 text-center text-sm text-neutral-500">
            ...
            <span>Обложка для страницы материала и карточек каталога</span>
          </div>
        )}
      </div>
