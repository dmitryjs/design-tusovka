...
-- Run AFTER cloud_patch_cart_orders.sql
...
comment on column public.orders.payment_provider is
  'Payment provider identifier, e.g. yookassa.';

comment on column public.orders.provider_payment_id is
  'Provider payment id (YooKassa payment.id).';

comment on column public.orders.payment_status is
  'Last known provider payment status (pending, succeeded, canceled, etc.).';

comment on column public.orders.entitlement_grant_error is
  'Set when payment succeeded but entitlement grant failed; requires manual retry.';

-- Idempotent paid order fulfillment (service role only)
...
comment on function public.fulfill_paid_order (uuid) is
  'Marks order paid (if pending) and grants direct_order entitlements idempotently. Service role only.';
